...
- **Decisión documental que puedo explicar:** Documenté el fallback de navegación como una limitación conocida, no como una funcionalidad garantizada: `caches.match("/")` se usa sin `await`, por lo que una coincidencia vacía puede terminar en `undefined`. También distinguí recuperación de solicitudes de sincronización de datos, que el proyecto no implementa.
- **Prueba o validación ejecutada y resultado real:** Con Node `v24.19.0`, ejecuté `node tests/service-worker.spec.ts` y terminó en PASS. Ejecuté `node tests/offline.spec.ts` y terminó en FAIL reproducible: el último caso obtuvo `undefined` donde esperaba un fallback. Intenté consultar y ejecutar `npm`, incluido `npm test`, pero PowerShell informó que `npm` no se reconoce; no afirmo resultados nuevos de `npm test`, build, verify ni del check público.
...
- **Limitación o hallazgo para integración:** `package.json` solo ejecuta los specs `.mjs`; `scripts/verify.mjs` los hereda y el workflow de Semana 3 no invoca los dos specs TypeScript. Además, el check público ejecutable de raíz conserva solo validaciones de Semanas 1–2. La corrección pertenece a la integración técnica y no se realizó en esta contribución.
